...
IF OBJECT_ID('#HTN12mo_indexcohort', 'U') IS NOT NULL
  DROP TABLE #HTN12mo_indexcohort;

IF OBJECT_ID('#HTN12mo_e0', 'U') IS NOT NULL
  DROP TABLE #HTN12mo_e0;

IF OBJECT_ID('#HTN12mo_t0', 'U') IS NOT NULL
	DROP TABLE #HTN12mo_t0;

IF OBJECT_ID('#HTN12mo_t1', 'U') IS NOT NULL
	DROP TABLE #HTN12mo_t1;

IF OBJECT_ID('#HTN12mo_t2', 'U') IS NOT NULL
	DROP TABLE #HTN12mo_t2;

IF OBJECT_ID('#HTN12mo_t3', 'U') IS NOT NULL
	DROP TABLE #HTN12mo_t3;

IF OBJECT_ID('#HTN12mo_matchcohort', 'U') IS NOT NULL
	DROP TABLE #HTN12mo_matchcohort;

IF OBJECT_ID('#HTN12mo_drug_seq', 'U') IS NOT NULL
	DROP TABLE #HTN12mo_drug_seq;

IF OBJECT_ID('#HTN12mo_drug_seq_summary', 'U') IS NOT NULL
	DROP TABLE #HTN12mo_drug_seq_summary;

IF OBJECT_ID('HTN12mo_OPTUM_summary', 'U') IS NOT NULL
	DROP TABLE HTN12mo_OPTUM_summary;

IF OBJECT_ID('HTN12mo_OPTUM_person_cnt', 'U') IS NOT NULL
	DROP TABLE HTN12mo_OPTUM_person_cnt;


IF OBJECT_ID('HTN12mo_OPTUM_seq_cnt', 'U') IS NOT NULL
	DROP TABLE HTN12mo_OPTUM_seq_cnt;
...
create table #HTN12mo_IndexCohort
(
...
INSERT INTO #HTN12mo_IndexCohort (PERSON_ID, INDEX_DATE, COHORT_END_DATE, OBSERVATION_PERIOD_START_DATE, OBSERVATION_PERIOD_END_DATE)
...
INSERT INTO #HTN12mo_E0
select ip.PERSON_ID, ip.INDEX_DATE, ip.COHORT_END_DATE
from #HTN12mo_IndexCohort ip
LEFT JOIN
(
	select co.PERSON_ID, co.CONDITION_CONCEPT_ID
	FROM CDM_OPTUM.dbo.condition_occurrence co
	JOIN #HTN12mo_IndexCohort ip on co.PERSON_ID = ip.PERSON_ID
	JOIN CDM_OPTUM.dbo.CONCEPT_ANCESTOR ca on co.CONDITION_CONCEPT_ID = ca.DESCENDANT_CONCEPT_ID and ca.ANCESTOR_CONCEPT_ID in (444094)
...
(
	...
	JOIN #HTN12mo_IndexCohort ip on de.PERSON_ID = ip.PERSON_ID
	JOIN CDM_OPTUM.dbo.CONCEPT_ANCESTOR ca on de.DRUG_CONCEPT_ID = ca.DESCENDANT_CONCEPT_ID and ca.ANCESTOR_CONCEPT_ID in (21600381,21601461,21601560,21601664,21601744,21601782)
...
(
	...
	JOIN #HTN12mo_IndexCohort ip on ce.PERSON_ID = ip.PERSON_ID
	JOIN CDM_OPTUM.dbo.CONCEPT_ANCESTOR ca on ce.CONDITION_CONCEPT_ID = ca.DESCENDANT_CONCEPT_ID and ca.ANCESTOR_CONCEPT_ID in (316866)
...
create table #HTN12mo_MatchCohort
(
	PERSON_ID bigint not null primary key,
	INDEX_DATE date not null,
	COHORT_END_DATE date not null,
...
INSERT INTO #HTN12mo_MatchCohort (PERSON_ID, INDEX_DATE, COHORT_END_DATE, OBSERVATION_PERIOD_START_DATE, OBSERVATION_PERIOD_END_DATE)
select c.person_id, c.index_date, c.cohort_end_date, c.observation_period_start_date, c.observation_period_end_date
FROM #HTN12mo_IndexCohort C
INNER JOIN
(
SELECT INDEX_DATE, OBSERVATION_END_DATE, PERSON_ID
FROM
(
	SELECT INDEX_DATE, OBSERVATION_END_DATE, PERSON_ID FROM #HTN12mo_E0
	INTERSECT
	SELECT INDEX_DATE, OBSERVATION_END_DATE, PERSON_ID FROM #HTN12mo_T0
	INTERSECT
	SELECT INDEX_DATE, OBSERVATION_END_DATE, PERSON_ID FROM #HTN12mo_T1
	INTERSECT
	SELECT INDEX_DATE, OBSERVATION_END_DATE, PERSON_ID FROM #HTN12mo_T2
	INTERSECT
	SELECT INDEX_DATE, OBSERVATION_END_DATE, PERSON_ID FROM #HTN12mo_T3
) TopGroup
) I 
...
insert into #HTN12mo_drug_seq (person_id, index_year, drug_concept_id, concept_name, drug_seq)
...
inner join #HTN12mo_MatchCohort c1
on de0.person_id = c1.person_id
where drug_concept_id in (select descendant_concept_id from CDM_OPTUM.dbo.concept_ancestor where ancestor_concept_id in (21600381,21601461,21601560,21601664,21601744,21601782))
...
create table #HTN12mo_drug_seq_summary
(
	index_year int,
	d1_concept_id int,
	d2_concept_id int,
...
create table CDM_OPTUM.dbo.HTN12mo_OPTUM_summary
(
...
insert into CDM_OPTUM.dbo.HTN12mo_OPTUM_summary (count_type, num_persons)
...
create table CDM_OPTUM.dbo.HTN12mo_OPTUM_person_cnt
(
	index_year int,
	num_persons int
);

insert into CDM_OPTUM.dbo.HTN12mo_OPTUM_person_cnt (index_year, num_persons)
select index_year, num_persons
from
(
select index_year, sum(num_persons) as num_persons
from #HTN12mo_drug_seq_summary
group by index_year
) t1
;
...
create table CDM_OPTUM.dbo.HTN12mo_OPTUM_seq_cnt
(
	index_year int,
	d1_concept_id int, 
	d2_concept_id int, 
...
TRUNCATE TABLE #HTN12mo_indexcohort;
DROP TABLE #HTN12mo_indexcohort;
TRUNCATE TABLE #HTN12mo_e0;
DROP TABLE #HTN12mo_e0;
TRUNCATE TABLE #HTN12mo_t0;
DROP TABLE #HTN12mo_t0;
TRUNCATE TABLE #HTN12mo_t1;
DROP TABLE #HTN12mo_t1;
TRUNCATE TABLE #HTN12mo_t2;
DROP TABLE #HTN12mo_t2;
TRUNCATE TABLE #HTN12mo_t3;
TRUNCATE TABLE #HTN12mo_matchcohort;
DROP TABLE #HTN12mo_matchcohort;
TRUNCATE TABLE #HTN12mo_drug_seq;
DROP TABLE #HTN12mo_drug_seq;
TRUNCATE TABLE #HTN12mo_drug_seq_summary;
DROP TABLE #HTN12mo_drug_seq_summary;