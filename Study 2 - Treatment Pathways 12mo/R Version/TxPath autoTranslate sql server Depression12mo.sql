...
IF OBJECT_ID('#Depression12mo_indexcohort', 'U') IS NOT NULL
  DROP TABLE #Depression12mo_indexcohort;

IF OBJECT_ID('#Depression12mo_e0', 'U') IS NOT NULL
  DROP TABLE #Depression12mo_e0;

IF OBJECT_ID('#Depression12mo_t0', 'U') IS NOT NULL
	DROP TABLE #Depression12mo_t0;

IF OBJECT_ID('#Depression12mo_t1', 'U') IS NOT NULL
	DROP TABLE #Depression12mo_t1;

IF OBJECT_ID('#Depression12mo_t2', 'U') IS NOT NULL
	DROP TABLE #Depression12mo_t2;

IF OBJECT_ID('#Depression12mo_t3', 'U') IS NOT NULL
	DROP TABLE #Depression12mo_t3;

IF OBJECT_ID('#Depression12mo_matchcohort', 'U') IS NOT NULL
	DROP TABLE #Depression12mo_matchcohort;

IF OBJECT_ID('#Depression12mo_drug_seq', 'U') IS NOT NULL
	DROP TABLE #Depression12mo_drug_seq;

IF OBJECT_ID('#Depression12mo_drug_seq_summary', 'U') IS NOT NULL
	DROP TABLE #Depression12mo_drug_seq_summary;

IF OBJECT_ID('Depression12mo_OPTUM_summary', 'U') IS NOT NULL
	DROP TABLE Depression12mo_OPTUM_summary;

IF OBJECT_ID('Depression12mo_OPTUM_person_cnt', 'U') IS NOT NULL
	DROP TABLE Depression12mo_OPTUM_person_cnt;


IF OBJECT_ID('Depression12mo_OPTUM_seq_cnt', 'U') IS NOT NULL
	DROP TABLE Depression12mo_OPTUM_seq_cnt;
...
create table #Depression12mo_IndexCohort
(
...
INSERT INTO #Depression12mo_IndexCohort (PERSON_ID, INDEX_DATE, COHORT_END_DATE, OBSERVATION_PERIOD_START_DATE, OBSERVATION_PERIOD_END_DATE)
...
create table #Depression12mo_E0
(
	PERSON_ID bigint not null primary key,
	INDEX_DATE date not null,
...
INSERT INTO #Depression12mo_E0
select ip.PERSON_ID, ip.INDEX_DATE, ip.COHORT_END_DATE
from #Depression12mo_IndexCohort ip
LEFT JOIN
(
	select co.PERSON_ID, co.CONDITION_CONCEPT_ID
	FROM CDM_OPTUM.dbo.condition_occurrence co
	JOIN #Depression12mo_IndexCohort ip on co.PERSON_ID = ip.PERSON_ID
	JOIN CDM_OPTUM.dbo.CONCEPT_ANCESTOR ca on co.CONDITION_CONCEPT_ID = ca.DESCENDANT_CONCEPT_ID and ca.ANCESTOR_CONCEPT_ID in (444094,432876,435783)
...
create table #Depression12mo_T0
(
	PERSON_ID bigint not null primary key,
	INDEX_DATE date not null,
	OBSERVATION_END_DATE date not null
...
(
	...
	JOIN #Depression12mo_IndexCohort ip on de.PERSON_ID = ip.PERSON_ID
	JOIN CDM_OPTUM.dbo.CONCEPT_ANCESTOR ca on de.DRUG_CONCEPT_ID = ca.DESCENDANT_CONCEPT_ID and ca.ANCESTOR_CONCEPT_ID in (21604686, 21500526)
...
create table #Depression12mo_T1
(
	PERSON_ID bigint not null primary key,
	INDEX_DATE date not null,
	OBSERVATION_END_DATE date not null
...
(
	...
	JOIN #Depression12mo_IndexCohort ip on ce.PERSON_ID = ip.PERSON_ID
	JOIN CDM_OPTUM.dbo.CONCEPT_ANCESTOR ca on ce.CONDITION_CONCEPT_ID = ca.DESCENDANT_CONCEPT_ID and ca.ANCESTOR_CONCEPT_ID in (440383)
...
create table #Depression12mo_T2
(
	PERSON_ID bigint not null primary key,
	INDEX_DATE date not null,
	OBSERVATION_END_DATE date not null
...
--find persons in indexcohort with >1 treatments in 4mo interval after index
create table #Depression12mo_T3
(
	PERSON_ID bigint not null primary key,
	INDEX_DATE date not null,
	OBSERVATION_END_DATE date not null
...
create table #Depression12mo_MatchCohort
(
	PERSON_ID bigint not null primary key,
	INDEX_DATE date not null,
	COHORT_END_DATE date not null,
...
INSERT INTO #Depression12mo_MatchCohort (PERSON_ID, INDEX_DATE, COHORT_END_DATE, OBSERVATION_PERIOD_START_DATE, OBSERVATION_PERIOD_END_DATE)
select c.person_id, c.index_date, c.cohort_end_date, c.observation_period_start_date, c.observation_period_end_date
FROM #Depression12mo_IndexCohort C
INNER JOIN
(
SELECT INDEX_DATE, OBSERVATION_END_DATE, PERSON_ID
FROM
(
	SELECT INDEX_DATE, OBSERVATION_END_DATE, PERSON_ID FROM #Depression12mo_E0
	INTERSECT
	SELECT INDEX_DATE, OBSERVATION_END_DATE, PERSON_ID FROM #Depression12mo_T0
	INTERSECT
	SELECT INDEX_DATE, OBSERVATION_END_DATE, PERSON_ID FROM #Depression12mo_T1
	INTERSECT
	SELECT INDEX_DATE, OBSERVATION_END_DATE, PERSON_ID FROM #Depression12mo_T2
	INTERSECT
	SELECT INDEX_DATE, OBSERVATION_END_DATE, PERSON_ID FROM #Depression12mo_T3
) TopGroup
) I 
...
create table #Depression12mo_drug_seq
(
	person_id bigint,
	index_year int,
	drug_concept_id int,
...
insert into #Depression12mo_drug_seq (person_id, index_year, drug_concept_id, concept_name, drug_seq)
...
inner join #Depression12mo_MatchCohort c1
on de0.person_id = c1.person_id
where drug_concept_id in (select descendant_concept_id from CDM_OPTUM.dbo.concept_ancestor where ancestor_concept_id in (21604686, 21500526))
...
create table #Depression12mo_drug_seq_summary
(
	index_year int,
	d1_concept_id int,
	d2_concept_id int,
...
create table CDM_OPTUM.dbo.Depression12mo_OPTUM_summary
(
...
insert into CDM_OPTUM.dbo.Depression12mo_OPTUM_summary (count_type, num_persons)
...
from #Depression12mo_MatchCohort
;
...
create table CDM_OPTUM.dbo.Depression12mo_OPTUM_person_cnt
(
	index_year int,
	num_persons int
);

insert into CDM_OPTUM.dbo.Depression12mo_OPTUM_person_cnt (index_year, num_persons)
select index_year, num_persons
from
(
select index_year, sum(num_persons) as num_persons
from #Depression12mo_drug_seq_summary
group by index_year
) t1
;
...
create table CDM_OPTUM.dbo.Depression12mo_OPTUM_seq_cnt
(
	index_year int,
	d1_concept_id int, 
	d2_concept_id int, 
...
insert into CDM_OPTUM.dbo.Depression12mo_OPTUM_seq_cnt (index_year, d1_concept_id, d2_concept_id, d3_concept_id, d4_concept_id, d5_concept_id, d6_concept_id, d7_concept_id, d8_concept_id, d9_concept_id, d10_concept_id, d11_concept_id, d12_concept_id, d13_concept_id, d14_concept_id, d15_concept_id, d16_concept_id, d17_concept_id, d18_concept_id, d19_concept_id, d20_concept_id, d1_concept_name, d2_concept_name, d3_concept_name, d4_concept_name, d5_concept_name, d6_concept_name, d7_concept_name, d8_concept_name, d9_concept_name, d10_concept_name, d11_concept_name, d12_concept_name, d13_concept_name, d14_concept_name, d15_concept_name, d16_concept_name, d17_concept_name, d18_concept_name, d19_concept_name, d20_concept_name, num_persons)
...
TRUNCATE TABLE #Depression12mo_indexcohort;
DROP TABLE #Depression12mo_indexcohort;
TRUNCATE TABLE #Depression12mo_e0;
DROP TABLE #Depression12mo_e0;
TRUNCATE TABLE #Depression12mo_t0;
DROP TABLE #Depression12mo_t0;
TRUNCATE TABLE #Depression12mo_t1;
DROP TABLE #Depression12mo_t1;
TRUNCATE TABLE #Depression12mo_t2;
DROP TABLE #Depression12mo_t2;
TRUNCATE TABLE #Depression12mo_t3;
TRUNCATE TABLE #Depression12mo_matchcohort;
DROP TABLE #Depression12mo_matchcohort;
TRUNCATE TABLE #Depression12mo_drug_seq;
DROP TABLE #Depression12mo_drug_seq;
TRUNCATE TABLE #Depression12mo_drug_seq_summary;
DROP TABLE #Depression12mo_drug_seq_summary;