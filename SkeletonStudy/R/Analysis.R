...
execute <- function(dbms, user, password, server, 
    ...
    # Read, parameterize and translate SQL
    parameterizedSql <- SqlReader::readSql(system.file(paste("sql/","sql_server",sep=""), 
                                                       "ParamterizedSql.sql", 
                                                       package="SkeletonStudy"))        
    renderedSql <- SqlRender::renderSql(parameterizedSql, 
                                        tableName = "drug_era")$sql    
    translatedSql <- SqlRender::translateSql(renderedSql, 
                                             sourceDialect = "sql server", 
                                             targetDialect = dbms)$sql
    
    # Execute SQL against CDM in DB
    result <- DBI::dbGetQuery(conn, translatedSql)    
    
    # Clean up and package result
    DBI::dbDisconnent(conn)
    setwd(currentWd)				
