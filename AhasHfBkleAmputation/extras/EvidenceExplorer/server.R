...
mainColumnNames <- c("<span title=\"Established cardiovascular disease\">Est. CVD</span>", 
                     "<span title=\"Prior exposure\">Prior Exposure</span>", 
...
shinyServer(function(input, output) {
  ...
  tableSubset <- reactive({
    idx <- resultsHois$comparison == input$comparison &
      ...
      resultsHois$establishedCvd %in% input$establishedCVd &
      resultsHois$priorExposure %in% input$priorExposure &
      resultsHois$timeAtRisk %in% input$timeAtRisk &
      resultsHois$evenType %in% input$evenType &
  ...
  output$table1Table <- renderDataTable({
    ...
    if (!is.null(bal)) {
      ...
      priorAhaBalance  <- readRDS(file.path("data", fileName))
