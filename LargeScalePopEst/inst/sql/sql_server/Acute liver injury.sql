...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (377604,4337543,196455,194990)and invalid_reason is null
...
LEFT JOIN
(
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (200763,196029,194087,4291005)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (192956,200763,4212540,197917,192353,192963,196456,4130518,4291005)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
) A on A.person_id = P.person_id and A.START_DATE BETWEEN P.OP_START_DATE AND P.OP_END_DATE AND A.START_DATE BETWEEN DATEADD(day,-365,P.START_DATE) and DATEADD(day,60,P.START_DATE)
GROUP BY p.event_id
