...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (4120275,317002,319041)and invalid_reason is null
...
LEFT JOIN
(
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (314432)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
