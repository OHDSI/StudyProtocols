...
calibrateEstimatesAndPvalues <- function(workFolder) {
    ...
    for (i in 1:nrow(exposureSummary)) {
        ...
        for (analysisId in c(1,3)){
            ...
            fileName <- file.path(calibrationFolder, paste0("negControls_a", analysisId, "_t", treatmentId, "_c", comparatorId, ".png"))
            analysisLabel <- "Crude"
            if (analysisId != 1) {
            ...
            EmpiricalCalibration::plotCalibrationEffect(logRrNegatives = negControls$logRr,
                                                        ...
                                                        xLabel = "Hazard ratio",
                                                        fileName = fileName)
            fileName <- file.path(calibrationFolder, paste0("negControls_a", analysisId, "_t", comparatorId, "_c", treatmentId, ".png"))
