...
mainColumnNames <- c("<span title=\"Event type\">Event type</span>",
                     "<span title=\"Time at risk\">Time at risk</span>",
                     "<span title=\"Propensity score strategy\">PS strategy</span>",
                     "<span title=\"Cana\">Canagliflozin history</span>",
                     #"<span title=\"Prior AP\">Prior AP</span>",                                                               
                     "<span title=\"Metformin Add On\">Metformin add-on</span>",
...
shinyServer(function(input, output) {
  ...
  tableSubset <- reactive({
    idx <- resultsHois$comparison == input$comparison &
      ...
      resultsHois$eventType %in% input$eventType &
      resultsHois$psStrategy %in% input$psStrategy &
      resultsHois$noCana %in% input$noCana &
      resultsHois$database %in% input$db &
      #resultsHois$priorAP %in% input$priorAP &
      resultsHois$metforminAddOn %in% input$metforminAddOn
  ...
  balance <- reactive({
    ...
    if (is.null(row) || row$database == "Meta-analysis") {
      ...
      print(paste("loading balance file: ", fileName))      
      data <- readRDS(file.path(dataLocation, fileName))
  ...
  output$powerTableCaption <- renderUI({
    ...
    if (!is.null(row)) {
      ...
      return(HTML(sprintf(text, row$targetDrug, row$comparatorDrug, tolower(row$psStrategy))))
    } else {
      return(NULL)
    }
  })
  ...
  output$table1Table <- renderDataTable({
    ...
    if (!is.null(bal)) {
      ...
      priorAhaBalance  <- readRDS(file.path(dataLocation, fileName))
  ...
  output$negativeControlPlot <- renderPlot({
    ...
    if (!is.null(row)) {
      ...
      fileName <- paste0("null_a",row$analysisId,"_t",row$targetId,"_c",row$comparatorId,"_",row$database,".rds")
      if (file.exists(file.path(dataLocation, fileName))) {
        null <- readRDS(file.path(dataLocation, fileName))
      } else {
        null <- NULL
      }
  ...
  output$kaplanMeierPlot <- renderPlot({
    ...
    if (!is.null(row)) {
      ...
      if (file.exists(file.path(dataLocation, fileName))) {
        plot <- readRDS(file.path(dataLocation, fileName))
