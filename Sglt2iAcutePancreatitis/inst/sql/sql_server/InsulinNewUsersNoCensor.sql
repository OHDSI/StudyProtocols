...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (21600713)and invalid_reason is null
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1529331,1530014,44816332,43013884,43526465,1594973,44785829,45774435,45774751,1583722,1597756,1560171,1559684,40239216,40170911,44506754,1510202,1502826,1525215,1516766,1547504,40166035,1580747,1502809,1502855,1515249,19122137,730548)and invalid_reason is null
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (1529331,1530014,44816332,43013884,43526465,1594973,44785829,45774435,45774751,1583722,1597756,1560171,1559684,40239216,40170911,44506754,1510202,1502826,1525215,1516766,1547504,40166035,1580747,1502809,1502855,1515249,19122137,730548)
  and c.invalid_reason is null
...
INTO #drugTarget
FROM @cdm_database_schema.DRUG_EXPOSURE de
JOIN #Codesets cs on cs.codeset_id = 5 
