...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4170742)and invalid_reason is null
...
WHERE (de.drug_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 3) 
	or de.drug_source_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 3))
