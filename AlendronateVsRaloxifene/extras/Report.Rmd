...
saveFigure <- function(filename, newFile, caption = NULL, number = NULL, width, height) {
  first <- newFile
...
table <-
  ...
  addtorow$command <- c("\\multicolumn{1}{c}{Data source} & \\multicolumn{1}{c}{Before} & \\multicolumn{1}{c}{After}  \\\\\n \\hline \\\\[-0.5em]",
                        "\\\\ [-0.5em]\\hline")  
...
## Vertebral fracture (intent-to-treat)
```{r, echo=FALSE, warning=FALSE}
...
## Atypical femural fracture (intent-to-treat)
```{r, echo=FALSE, warning=FALSE}
...
cowplot::plot_grid(gridExtra::arrangeGrob(hip1$data_table, blank, hip1$p, ncol = 3, widths = c(1,0.1,1)),
                   ...
                   gridExtra::arrangeGrob(blank),
                   gridExtra::arrangeGrob(atyp1$data_table, blank, atyp1$p, ncol = 3, widths = c(1,0.1,1)), 
                   nrow = 5, labels=c('A)',"",'B)',"",'C)'), label_fontface = "plain", vjust = 1,
