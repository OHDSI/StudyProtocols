...
*****/
  ...
  CREATE GLOBAL TEMPORARY TABLE T2DM_drug_seq_summary_temp
   (
    index_year int,
  	d1_concept_id int,
  	d2_concept_id int,
...
BEGIN
  EXECUTE IMMEDIATE 'TRUNCATE TABLE  T2DM_source_name_population_summary';
  EXECUTE IMMEDIATE 'DROP TABLE  T2DM_source_name_population_summary';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE != -942 THEN
      RAISE;
