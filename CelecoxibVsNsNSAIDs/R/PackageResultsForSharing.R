...
packageResults <- function(outputFolder = outputFolder) {
    ...
    for (analysisId in unique(analysisSummary$analysisId)){
        negControlSubset <- analysisSummary[analysisSummary$analysisId == analysisId & analysisSummary$outcomeId %in% negControlCohortIds, ]
        negControlSubset <- negControlSubset[!is.na(negControlSubset$logRr) & negControlSubset$logRr != 0,]
        posControlSubset <- analysisSummary[analysisSummary$analysisId == analysisId & !(analysisSummary$outcomeId %in% negControlCohortIds), ]
        posControlSubset <- posControlSubset[!is.na(posControlSubset$logRr) & posControlSubset$logRr != 0,]
        ...
        if (nrow(negControlSubset) > 10){
            EmpiricalCalibration::plotCalibrationEffect(negControlSubset$logRr,
                                                        ...
                                                        fileName = file.path(exportFolder, paste("CalEffect_a",analysisId,".png",sep = "")))
            EmpiricalCalibration::plotCalibration(negControlSubset$logRr,
                                                  negControlSubset$seLogRr,
                                                  useMcmc = TRUE,
                                                  fileName = file.path(exportFolder, paste("Calibration_a",analysisId,".png",sep = "")))
        }
    }
