...
doEmpiricalCalibration <- function(outputFolder){
    ...
    #ps <- readRDS(unique(outcomeReference$sharedPsFile)[2])
    #CohortMethod::plotPs(ps)

    om <- readRDS(outcomeReference$outcomeModelFile[outcomeReference$analysisId == 2 & outcomeReference$outcomeId == 10])
    summary(om)
