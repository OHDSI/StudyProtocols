exportInitialSGLT2iDosage <- function(connectionDetails,
                                   packageName,
  ...
  sql <- SqlRender::loadRenderTranslateSql(sqlFilename = "exportInitialSGLT2iDosage.sql",
  ...
  if(i == last){
    file = paste0(study,"_Results_Initial_SGLT2i_Dossage_",Sys.Date(),".xlsx")
    if(file.exists(file)){
      file.remove(file)
    }

    #Table 1
    sql <- paste0("SELECT * FROM ",Sys.getenv("writeTo"),".",target_table, " ORDER BY DB, COHORT_DEFINITION_ID, DOSE_STRENGTH, STAT_TYPE")
