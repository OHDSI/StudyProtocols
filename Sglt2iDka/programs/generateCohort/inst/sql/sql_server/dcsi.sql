IF OBJECT_ID('@covariate_table', 'U') IS NOT NULL
	DROP TABLE @covariate_table;

/*https://github.com/OHDSI/FeatureExtraction/blob/143ac1ea48c04a8befb73f0dde57237993aa92b9/inst/sql/sql_server/CreateCovAnalysisRefTables.sql*/
...
/*https://github.com/OHDSI/FeatureExtraction/blob/master/inst/sql/sql_server/Dcsi.sql*/
/*The Diabetes Comorbidity Severity Index (DCSI) using all conditions prior to the window end.*/
...
FROM (
  ...
  {@aggregated} ? {
    ...
    GROUP BY /*^^^^^^^^^*/@cohort_definition_id, cohort.@row_id_field,
    dcsi_category
  }
) temp
{@aggregated} ? {
  ...
  GROUP BY /*^^^^^^^^^*/@cohort_definition_id, row_id
}
;
