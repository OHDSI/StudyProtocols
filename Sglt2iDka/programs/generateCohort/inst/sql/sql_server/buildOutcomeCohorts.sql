/*
Based on this cohort shell: cohortdefinition/7062

How to fix ATLAS code to run in this program:
1)  Add this to the top of the SQL:
DELETE FROM @target_database_schema.@target_cohort_table
WHERE cohort_definition_id = @target_cohort_id;

2) Replace CODSET_ID = 0 with the following (this is the cohort of interest code):
  SELECT CONCEPT_ID FROM @target_database_schema.@codeList
  WHERE CODE_LIST_NAME IN (	'DKA' )

3) Search for:
AND V.visit_concept_id in (9203,262,9201)

replace with:
AND V.visit_concept_id in (
  {@target_cohort_id == 200}?{9203,262,9201}
  {@target_cohort_id == 201}?{9201}
  {@target_cohort_id == 900}?{9203,262,9201}
  {@target_cohort_id == 901}?{9201}
)

4) Search for "30"" and replace with, there should be 2:
@gap_days
...
(
	SELECT
		person_id
		, DATEADD(day,-1 * @gap_days, event_date)  as end_date
	FROM
	(
		SELECT
			person_id
		...
		(
			...
			SELECT
				person_id
				, DATEADD(day,@gap_days,end_date) as end_date
				, 1 AS event_type
				, NULL
			FROM #cohort_rows
		) RAWDATA
