...
**Table 1a**. Number of subjects, follow-up time (in days), number of outcome events, and event incidence rate (IR) per 1,000 patient years (PY) in the target (%target%) and comparator (%comparator%) group after matching, as well as the minimum detectable relative risk (MDRR).
```{r, echo = FALSE}
...
if(!isMetaAnalysis){
  cat("**Table 1b**. Time (days) at risk distribution expressed as mean, standard deviation (SD), minimum (min), median, and maximum (max) in the target (%target%) and comparator (%comparator%) cohort after matching")
}
```
```{r, echo = FALSE, results='asis'}
if(!isMetaAnalysis){
...
**Table 2**. Select characteristics before and after matching, showing the percentage of subjects with the characteristics in the target (%target%) and comparator (%comparator%) group, as well as the standardized difference of the means.")
}
```
```{r, echo = FALSE, results='asis'}
if(!isMetaAnalysis){
  ...
  kable(table, 
    ...
    add_header_above(c("", "Before matching" = 3, "After matching" = 3))  %>%
...
if(!isMetaAnalysis){
  ...
  ggplot2::ggplot(balance,
    ...
    ggplot2::scale_x_continuous("Std. diff. before matching", limits = limits) +
    ggplot2::scale_y_continuous("Std. diff. after matching", limits = limits)
}
```
```{r, echo = FALSE, results='asis'}
if(!isMetaAnalysis){
  cat("**Figure 2**. Covariate balance before and after matching. Each dot represents the standardized difference of means for a single covariate before and after matching on the propensity score.")
}
```
...
if (sum(!is.na(ncs$seLogRr)) < 5) {
  cat("Unfortunately, there were not enough data to compute a systematic error distribution.") 
}
```
...
```{r, echo = FALSE, results='asis'}
if (sum(!is.na(ncs$seLogRr)) >= 5) {
  cat("**Figure 3**. Negative control estimates. Each blue dot represents the estimated hazard ratio and standard error (related to the width of the confidence interval) of each of the negative control outcomes. The yellow diamond indicated the outcome of interest. Estimates below the dashed line have nominal p < .05. Estimates in the orange area have calibrated p < .05. The red band indicated the 95% credible interval around the boundary of the orange area.")
}
```
